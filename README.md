...
## 实验结果汇总
以下表格展示了不同超参配置（图像高度与 AVIF 质量）下的重建信噪比 (PSNR)。
测试基于示例音频 `demo/input.wav`。

| 配置名称 | 图像高度 (Height/Mel Bands) | AVIF 质量 (Quality) | PSNR (dB) | 备注 |
| :--- | :--- | :--- | :--- | :--- |
| **Q90 (默认)** | 192 | 90 | **25.53** | 平衡配置 (脚本默认) |
| **H512** | 512 | 90 | **39.70** | 高保真 (高频细节更丰富) |
| **H256** | 256 | 90 | **30.94** | 较好的音质与体积平衡 |
| **H80** | 80 | 90 | **21.24** | 低高度导致高频丢失 |
| **H128 (基准)** | 128 | 90 | **22.11** | 128 Mel 频带基准 |
| **Q95** | 128 | 95 | **22.12** | 提高质量参数对低频带提升有限 |
| **Q85** | 128 | 85 | **22.10** | 质量参数微调 |
| **Q70** | 128 | 70 | **22.03** | 低质量压缩 |

> **结论**: 增加图像高度 (Mel 频带数) 对提升音频重建质量最为显著 (从 21dB 提升至近 40dB)，而单纯提高 AVIF 压缩质量 (Q值) 在低频带分辨率受限时提升幅度较小。
